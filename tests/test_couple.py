...
def test_lpjml_coupler(test_path):
    ...
    # test subsetted dataset
    sub_dataset = outputs.sel(variable="cftfrac")
    assert all(
        sub_dataset.coords["band"].values == outputs.coords["band (cftfrac)"].values
    )
    assert set(sub_dataset.dims) == {"cell", "band (cftfrac)", "band", "time"}
